...
- `CGEventTap` で左クリックを横取りし、押した瞬間の指の本数で「中クリックに変換 / 握りつぶして ⇧⌘5 を送る / そのまま通す」を決める
  ...
  - 左クリックにはどのトラックパッドで押されたかの情報が無いので、Force Touch のトラックパッドでは指の押す力から決める。押す力が 50 以上のトラックパッド（押し始めから 0.2 秒以内）をクリック元とし、その指の本数で判定する。押し続けているだけのトラックパッドは、0.2 秒を過ぎるとクリック元にならない。指を置いているだけ（押す力 0〜35 程度）ならクリック元にならないので、指を置いたままマウスでクリックしても変換されない。2台使っていても、クリックした方の本数で判定する
