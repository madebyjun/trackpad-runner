...
- キーチェーンに自己署名のコード署名証明書 `TrackpadRunner Dev` があれば、それで署名します（キーチェーンアクセス > 証明書アシスタント > 証明書を作成、固有名の種類「自己署名ルート」、証明書のタイプ「コード署名」）。この場合、ビルドし直しても権限は外れません。証明書が無ければアドホック署名になり、ビルドし直すたびに権限が外れます。その場合は、一覧からいったん削除して許可し直してください。
...
- `CGEventTap` で左クリックを横取りし、押した瞬間の指の本数で「中クリックに変換 / 握りつぶして ⇧⌘5 を送る / そのまま通す」を決める
  ...
  - 左クリックにはどのトラックパッドで押されたかの情報が無いので、Force Touch のトラックパッドでは指の押す力から決める。押す力が 50 以上のトラックパッドをクリック元とし、その指の本数で判定する。指を置いているだけ（押す力 0〜35 程度）ならクリック元にならないので、指を置いたままマウスでクリックしても変換されない。2台使っていても、クリックした方の本数で判定する
  - 押す力のフレームが左クリックより遅れて届くことがあるので、3本以上触れている Force Touch のトラックパッドがあるときだけ、最大 30ms 待ってから判定する
  - 3本以上の指のうち1本を強めに押さえたまま（クリックはせずに）マウスでクリックすると、変換されることがある。ゆっくり押し込むクリックを取りこぼさないことを優先して、押し始めからの時間では打ち切っていない
